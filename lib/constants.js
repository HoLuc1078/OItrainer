...
const GET_TALENT_Probability = 0.12;
...
/* 退队保护：压力到阈值后不会当场退队，而是先挂"退队倾向"，过一周才真的退队 */
const QUIT_PRESSURE_TENDENCY = 90;   // 压力达到该值 → 产生退队倾向（警告）
const QUIT_PRESSURE_HARD = 100;      // 压力顶到该值 → 只给退队保护，缓冲过一周后仍顶着才会真退队
const QUIT_PROTECT_WEEKS = 1;        // 退队保护的缓冲周期数（"再过一周才退队"）
...
CHUJINGFAZHI = 0.6;//出境集训时【每名学生】触发国家专属事件的概率。
                     //注意：不是"每支队伍触发一次"，而是每次集训对每名参加的学生各掷一次骰子，
                     //所以一次集训可能有多人分别遇到不同的事情，也可能一个人都没遇上。
OVERSEAS_INCIDENT_PROB = 0.42;//出境集训途中发生"意外"（负面事件）的概率，每次集训最多发生一起
/* 劝退消耗声誉 */
...
const HIDDEN_TALENTS = [
    // 魔改隐藏天赋：prob 均为 0，不会在训练中随机出现，
    // 只能通过「出境集训 → 天赋激发」花钱博取，或在个别国家的集训彩蛋中觉醒。
    // 命名一律走梗（懂的都懂），描述一律"点到为止"，像愚人节彩蛋一样不写真实数值。
    '嬲选手',
    '珂朵莉',
    '世界上最幸福的女孩',
    '样例过了就是过了',
...
// 隐藏天赋的旧名字 → 新名字。旧存档里若还留着老名字，读档时会被自动改写。
const LEGACY_TALENT_ALIASES = {
    '蒙的全对': '样例过了就是过了',
    '算法之神': '暴力出奇迹',
    'AK机器': '你怎么知道我 AK 了',
    '心如止水': '稳如老狗',
    '时之沙漏': 'Deadline 是第一生产力',
    '天命之子': '欧皇'
};

/* =========== 全局增幅变量（数值平衡总开关） =========== */
// 这些增幅在最终应用影响时作为乘数，默认为 1.000。
// 魔改版整体上调了难度：成长变慢、压力涨得更快、分数线和题目难度都更高、花销更贵。
// ⚠️ 下面这组是【普通难度】的基准值，实际生效值 = 基准 × 当前难度的档位系数，
//    由 applyDifficultyBalance() 在开局 / 读档时算好写回同名全局变量。
const BASE_TRAINING_EFFECT_MULTIPLIER = 0.92;   // 训练效果增幅（训练后属性增加）
const BASE_OUTFIT_EFFECT_MULTIPLIER = 0.95;     // 集训效果增幅（集训后属性增加）
const BASE_PRESSURE_INCREASE_MULTIPLIER = 1.15; // 压力增加量增幅
const BASE_PASS_LINE_MULTIPLIER = 1.08;         // 分数线增幅
const BASE_DIFFICULTY_MULTIPLIER = 1.06;        // 题目难度增幅
const BASE_COST_MULTIPLIER = 1.10;              // 经费消耗增幅

TRAINING_EFFECT_MULTIPLIER = BASE_TRAINING_EFFECT_MULTIPLIER;
OUTFIT_EFFECT_MULTIPLIER = BASE_OUTFIT_EFFECT_MULTIPLIER;
PRESSURE_INCREASE_MULTIPLIER = BASE_PRESSURE_INCREASE_MULTIPLIER;
PASS_LINE_MULTIPLIER = BASE_PASS_LINE_MULTIPLIER;
DIFFICULTY_MULTIPLIER = BASE_DIFFICULTY_MULTIPLIER;
COST_MULTIPLIER = BASE_COST_MULTIPLIER;

/* =========== 难度档位（简单 / 普通 / 专家） ===========
 * 想整体调松紧只改这一张表：每个系数都会乘到上面那组基准值上。
 *   budgetMult   开局经费乘数（不经过全局变量，直接在 initGame 里用）
 *   abilityDelta 初始能力加减（同上）
 *   train        训练收益
 *   outfit       集训收益
 *   pressure     压力增长
 *   passLine     分数线
 ...
 *   talent       天赋获取概率
 *
 * 设计意图：简单档只是「钱多一点、起点高一点」，玩法节奏不变；
 *          专家档则是全方位的紧 —— 钱少、起点低、练得慢、压得快、线还高。
 */
const DIFFICULTY_BALANCE = {
  1: { name: '简单', budgetMult: 1.50, abilityDelta: +20, train: 1.00, outfit: 1.00, pressure: 1.00, passLine: 1.00, problem: 1.00, cost: 1.00, talent: 1.00 },
  2: { name: '普通', budgetMult: 1.00, abilityDelta: 0,   train: 1.00, outfit: 1.00, pressure: 1.00, passLine: 1.00, problem: 1.00, cost: 1.00, talent: 1.00 },
  3: { name: '专家', budgetMult: 0.42, abilityDelta: -24, train: 0.88, outfit: 0.90, pressure: 1.12, passLine: 1.05, problem: 1.06, cost: 1.08, talent: 0.75 }
};

/** 取某一档难度的平衡系数（越界一律按普通档处理） */
function getDifficultyBalance(difficulty) {
  let d = Number(difficulty);
  if (!isFinite(d)) d = 2;
  d = Math.min(3, Math.max(1, Math.round(d)));
  return DIFFICULTY_BALANCE[d] || DIFFICULTY_BALANCE[2];
}

/**
 * 按当前难度重算所有全局增幅变量。
 * 必须在【开局】和【读档】之后各调用一次 —— 这些变量是全局的，
 * 刷新页面会回到基准值，不重新应用的话读档进来的专家档就会退化成普通档。
 * @returns {Object} 生效的档位系数
 */
function applyDifficultyBalance(difficulty) {
  const p = getDifficultyBalance(difficulty);
  TRAINING_EFFECT_MULTIPLIER = BASE_TRAINING_EFFECT_MULTIPLIER * p.train;
  OUTFIT_EFFECT_MULTIPLIER = BASE_OUTFIT_EFFECT_MULTIPLIER * p.outfit;
  PRESSURE_INCREASE_MULTIPLIER = BASE_PRESSURE_INCREASE_MULTIPLIER * p.pressure;
  PASS_LINE_MULTIPLIER = BASE_PASS_LINE_MULTIPLIER * p.passLine;
  DIFFICULTY_MULTIPLIER = BASE_DIFFICULTY_MULTIPLIER * p.problem;
  COST_MULTIPLIER = BASE_COST_MULTIPLIER * p.cost;
  try {
    if (typeof window !== 'undefined') {
      window.__OI_DIFFICULTY_PROFILE = p;
      window.__OI_EFFECTIVE_MULTIPLIERS = {
        TRAINING_EFFECT_MULTIPLIER: TRAINING_EFFECT_MULTIPLIER,
        OUTFIT_EFFECT_MULTIPLIER: OUTFIT_EFFECT_MULTIPLIER,
        PRESSURE_INCREASE_MULTIPLIER: PRESSURE_INCREASE_MULTIPLIER,
        PASS_LINE_MULTIPLIER: PASS_LINE_MULTIPLIER,
        DIFFICULTY_MULTIPLIER: DIFFICULTY_MULTIPLIER,
        COST_MULTIPLIER: COST_MULTIPLIER
      };
    }
  } catch (e) { /* ignore */ }
  console.log('[难度平衡] ' + p.name + '档生效：' + JSON.stringify(p));
  return p;
}
...
/* 出境集训的国家/地区数据与专属效果已迁移至 lib/countries.js */
// 省份/国家的季节气候与极端天气统一由 lib/climate.js 的 CLIMATE_TABLE 提供，
// 经 lib/provinces.js 的 climateKey 关联；此处不再维护旧的 climate 兜底数据。
