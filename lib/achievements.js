...
(function (global) {
  ...
  /* ==================== 跨局成就档案（持久化） ====================
   * 成就不能只跟着存档走 —— 存档一删、换一局，之前解锁的就全没了。
   * 这里用 localStorage 单独存一份「跨局累计档案」：
   *   {
   *     version: 1,
   *     runs: 12,                       // 一共开过多少局
   *     unlocked: {                     // 每个成就的解锁记录
   *       '<id>': { first: <ts>, last: <ts>, runs: 3 }
   *     }
   *   }
   * 判定"是否已解锁"以档案为准（并集上本局记录，兼容老存档），
   * 这样成就才是真正持久的：解锁一次，之后每一局都算数。
   */
  const PROFILE_KEY = 'oi_achievements_profile_v1';

  let _profileCache = null;

  function __emptyProfile() {
    return { version: 1, runs: 0, unlocked: {} };
  }

  /** 读取档案（带缓存；localStorage 不可用时退化成内存档案） */
  function __profile() {
    ...
    if (!p || typeof p !== 'object') p = __emptyProfile();
  ...
  /** 写回档案 */
  function __persistProfile() {
    try {
      if (typeof localStorage !== 'undefined') {
        localStorage.setItem(PROFILE_KEY, JSON.stringify(__profile()));
      }
    } catch (e) { /* 隐私模式 / 配额满：不写盘也要保持内存里的一致性 */ }
  }

  /** 把档案里的某个成就标记为解锁；返回是否"首次写入" */
  function __profileUnlock(id) {
    try {
      const p = __profile();
      const now = Date.now();
      const rec = p.unlocked[id];
      if (rec) {
        rec.last = now;
        rec.runs = Number(rec.runs || 1) + 1;
      } else {
        p.unlocked[id] = { first: now, last: now, runs: 1 };
      }
      __persistProfile();
      return !rec;
    } catch (e) { return false; }
  ...
  /** 确保本局已经解锁的成就也进了档案（读老存档 / 分享回来的存档时用） */
  function __profileSyncFromRun(game) {
    try {
      const g = game || (typeof window !== 'undefined' ? window.game : null);
      if (!g || !Array.isArray(g.hiddenAchievements)) return;
      let changed = false;
      g.hiddenAchievements.forEach(function (x) {
        const id = (typeof x === 'string') ? x : (x && x.id);
        if (!id) return;
        const p = __profile();
        if (!p.unlocked[id]) { p.unlocked[id] = { first: Date.now(), last: Date.now(), runs: 1 }; changed = true; }
      });
      if (changed) __persistProfile();
    } catch (e) { /* ignore */ }
  }
  ...
  const AchievementManager = {
    ...
    /* ---------- 跨局累计（localStorage 档案，持久化） ---------- */

    /** 档案里已解锁的全部 id */
    profileIds() {
      try { return Object.keys(__profile().unlocked); } catch (e) { return []; }
    },
    /** 单个成就在档案里记录的详情（首次 / 最近解锁时间、解锁过的局数） */
    profileRecord(id) {
      try { return __profile().unlocked[id] || null; } catch (e) { return null; }
    },
    /** 一共开过多少局 */
    profileRuns() {
      try { return Number(__profile().runs || 0); } catch (e) { return 0; }
    },
    /** 某一局开始时调用：累计局数 +1（每局只算一次） */
    markRunStart() {
      try {
        const p = __profile();
    ...
    /** 把本局的解锁记录补进档案（读档 / 导入分享时用） */
    syncProfileFromRun(game) { __profileSyncFromRun(game); },
    /**
     * 清空跨局档案（成就面板里的「清空记录」按钮）。
     * @param {Object} [game] 传了就顺手把本局的解锁记录也清掉，做到真正意义上的重置
     */
    resetProfile(game) {
      try {
        _profileCache = __emptyProfile();
        __persistProfile();
        const g = game || (typeof window !== 'undefined' ? window.game : null);
        if (g && Array.isArray(g.hiddenAchievements)) g.hiddenAchievements = [];
        return true;
      } catch (e) { return false; }
    },
    ...
    /**
     * 判定已解锁的完整集合 = 本局 ∪ 跨局档案。
     * 面板、徽章、checkAll 都用它 —— 所以成就"解锁一次，之后每一局都算数"。
     */
    allUnlockedIds(game) {
      const set = new Set(this.profileIds());
      this.unlockedIds(game).forEach(function (id) { set.add(id); });
      return Array.from(set);
    },
    /** 是否已解锁（跨局） */
    has(id, game) { return this.allUnlockedIds(game).indexOf(id) >= 0; },
    /** 跨局累计解锁数量（可按类别筛选） */
    count(game, kind) {
      const ids = this.allUnlockedIds(game);
      if (!kind) return ids.length;
      return ALL_ACHIEVEMENTS.filter(a => a.kind === kind && ids.indexOf(a.id) >= 0).length;
    },
    /** 本局解锁数量 */
    countThisRun(game, kind) {
      const ids = this.unlockedIds(game);
      if (!kind) return ids.length;
      return ALL_ACHIEVEMENTS.filter(a => a.kind === kind && ids.indexOf(a.id) >= 0).length;
    },

    /** 已解锁数量（跨局累计；可按类别筛选） */
    countOf(game, kind) { return this.count(game, kind); },

    /** 按分类统计解锁进度（跨局累计；可按类别筛选） */
    countsByCategory(game, kind) {
      const unlocked = this.allUnlockedIds(game);
      const out = {};
    ...
    unlock(id, game) {
      ...
      try {
        ...
        // 面板开着的时候顺手刷新一下，不用关掉重开
        try { if (typeof window !== 'undefined' && typeof window.oiAchRefresh === 'function' && document.querySelector('.ach-panel')) window.oiAchRefresh(); } catch (e) { }
        return true;
    ...
    renderPanelHtml(game) {
      const g = game || (typeof window !== 'undefined' ? window.game : null);
      // 面板展示用「跨局累计」口径：解锁一次，之后每一局都算数
      const unlocked = this.allUnlockedIds(g);
      const runIds = this.unlockedIds(g);
      ...
      html += '<div class="ach-head">🏅 成就（跨局累计）' + unlocked.length + ' / ' + ALL_ACHIEVEMENTS.length + '</div>';
      html += '<div class="ach-sub small muted">成就记录保存在浏览器本地，换存档、重开一局都不会丢；已解锁的成就不再重复弹提示。' +
        '　本局新解锁：<strong>' + runIds.length + '</strong> 个 · 累计进行 ' + this.profileRuns() + ' 局</div>';
      ...
      // 底部：档案说明 + 清空记录
      html += '<div class="ach-foot small muted">' +
        '成就记录存在 <code>localStorage["' + PROFILE_KEY + '"]</code>，与存档相互独立。' +
        '<button type="button" class="ach-reset" onclick="oiAchReset()">清空成就记录</button>' +
        '</div>';
    ...
    /** 一句话汇报（结算页 / 控制台用） */
    summaryText(game) {
      return '成就（累计）' + this.count(game) + ' / ' + ALL_ACHIEVEMENTS.length +
        '（外显 ' + this.count(game, 'public') + '/' + this.total('public') +
        ' · 隐藏 ' + this.count(game, 'hidden') + '/' + this.total('hidden') +
        '；本局新解锁 ' + this.countThisRun(game) + '）';
    }
  };

  /** 刷新已经打开着的成就面板（解锁后不用重新打开就能看到变化） */
  function oiAchRefresh() {
    try {
      const root = document.querySelector('.ach-panel');
      if (!root) return;
      const active = root.querySelector('.ach-tab.ach-tab-on');
      const kind = (active && active.dataset.achTab) || 'public';
      const wrap = root.parentElement;
      if (!wrap) return;
      const tmp = document.createElement('div');
      tmp.innerHTML = AchievementManager.renderPanelHtml();
      const fresh = tmp.firstElementChild;
      if (!fresh) return;
      root.parentElement.replaceChild(fresh, root);
      oiAchTab(kind);
    } catch (e) { console.error('oiAchRefresh failed', e); }
  }

  /** 清空跨局成就档案（面板底部的按钮；会二次确认） */
  function oiAchReset() {
    try {
      const n = AchievementManager.count();
      if (n > 0 && typeof confirm === 'function' && !confirm('确定要清空成就记录吗？\n\n已经解锁的 ' + n + ' 个成就（含本局记录）将全部变回未解锁状态，且无法恢复。')) return;
      AchievementManager.resetProfile();
      try { if (typeof updateAchievementBadge === 'function') updateAchievementBadge(); } catch (e) { }
      oiAchRefresh();
    } catch (e) { console.error('oiAchReset failed', e); }
  }
  ...
  global.oiAchRefresh = oiAchRefresh;
  global.oiAchReset = oiAchReset;
  ...
  window.oiAchRefresh = oiAchRefresh;
  window.oiAchReset = oiAchReset;
