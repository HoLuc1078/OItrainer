...
(function (global) {
  ...
  const AchievementManager = {
    ...
    renderPanelHtml(game) {
      ...
      const byCat = this.countsByCategory(g);

      let html = '<div class="ach-panel">';
      html += '<div class="ach-head">🏅 隐藏成就 ' + unlocked.length + ' / ' + this.total() + '</div>';
      html += '<div class="small muted" style="margin-bottom:10px">这些成就不会被提前剧透，解锁后才会亮出真面目。带着进度条的那些，看一眼就知道还差多少。</div>';
      ...
      CATEGORY_ORDER.forEach(function (c) {
        const defs = HIDDEN_ACHIEVEMENTS.filter(d => (d.category || '彩蛋') === c);
        ...
        defs.forEach(function (def) {
          ...
          let progressHtml = '';
          if (!on) {
            const p = AchievementManager.progressOf(def.id, g);
            if (p) {
    ...
    summaryText(game) {
      const unlocked = this.unlockedIds(game);
      return '隐藏成就 ' + unlocked.length + ' / ' + this.total();
    }
  };
