...
function buildOverseasContext(students, current, countryName){
  ...
  function pickOne(){
    ...
    return students[Math.floor(getRandom() * students.length)];
  }
  return {
    country: countryName,
    uniformInt: function(a, b){ return (typeof uniformInt === 'function') ? uniformInt(a, b) : (a + Math.floor(getRandom() * (b - a + 1))); },
    pick: pickOne,
    ...
    talent: function(s, name, prob){
      ...
      if(getRandom() >= (typeof prob === 'number' ? prob : 0.1)) return false;
...
function applyOverseasIncident(countryName, participantNames){
  try{
    ...
    if(!(getRandom() < prob)) return { count: 0, text: null };

    var happened = [];
    var texts = [];
    for(var round = 0; round < 2; round++){
      if(round === 1 && !(getRandom() < 0.15)) break; // 雪上加霜的概率很低
...
function applyOverseasCountryEffect(countryName, participantNames) {
  try {
    ...
    for (var i = 0; i < students.length; i++) {
      var s = students[i];
      if (perStudentProb < 1 && getRandom() >= perStudentProb) continue; // 每名学生各自掷骰
