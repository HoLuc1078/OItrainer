...
function getAdjustedBudgetForProvince(provinceId, difficulty) {
  const p = PROVINCES[provinceId];
  if (!p) return 100000;
  // 倍率统一取自 constants.js 的 DIFFICULTY_BALANCE，避免两处数字对不上
  const prof = (typeof getDifficultyBalance === 'function') ? getDifficultyBalance(difficulty) : null;
  const mult = prof ? Number(prof.budgetMult) : (difficulty === 1 ? 1.5 : difficulty === 3 ? 0.5 : 1.0);
  return Math.floor(p.baseBudget * mult);
}
...
function getProvinceAbilityRange(provinceId, difficulty) {
  ...
  // 初始能力也走难度档位表（constants.js 的 DIFFICULTY_BALANCE.abilityDelta）
  const prof = (typeof getDifficultyBalance === 'function') ? getDifficultyBalance(difficulty) : null;
  if (prof) {
    const delta = Number(prof.abilityDelta) || 0;
    if (delta !== 0) { min_val += delta; max_val += delta; }
  } else {
