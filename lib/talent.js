  ...
  checkAndHandleTalentLoss(student) {
    ...
    registerDefaultTalents(game, utils){
      ...
      this.registerTalent({
        ...
        this.registerTalent({
            name: '世界上最幸福的女孩',
            description: '我永远喜欢珂朵莉！每参加一场正式比赛，全能力永久 +10%。',
            color: '#ff0000',
            prob: 0.0,
            beneficial: true,
            handler: function (student, eventName, ctx) {
        ...
        // 蒙的全对：每次思考阶段小概率直接 AC 当前题目
        this.registerTalent({
            name: '蒙的全对',
            description: '运气也是实力的一部分：比赛中有概率随手一交就是满分。',
            color: '#7C4DFF',
            prob: 0.0,
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    ...
                    if (Math.random() < 0.02) {
                        return { action: 'auto_pass_problem', message: student.name + ' 蒙的全对：随手一交竟然 AC 了！' };
                    }
                } catch (e) { console.error('蒙的全对 天赋错误', e); }
                return null;
            }
        });

        // 算法之神：每通过一个子任务，本场比赛思维能力临时 +6%（最多 12 层）
        this.registerTalent({
            name: '算法之神',
            description: '越打越顺：比赛中每通过一个子任务档位，思维能力临时 +6%（最多 12 层）。',
            color: '#FF6D00',
            prob: 0.0,
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    ...
                    student._talent_state = student._talent_state || {};
                    var stack = Number(student._talent_state['算法之神'] || 0);
                    if (stack >= 12) return null;
                    student._talent_state['算法之神'] = stack + 1;
                    var bonus = Number(student._base_thinking || 0) * 0.06;
                    student.applyTempModifier('thinking', student.getTempModifier('thinking') + bonus);
                    return '算法之神发动（第' + (stack + 1) + '层）：思维 +6%（赛中临时）';
                } catch (e) { console.error('算法之神 天赋错误', e); }
                return null;
            }
        });

        // AK 机器：比赛开始全知识点 +35%，此后每 AC 一题再 +10%（最多 5 次）
        this.registerTalent({
            name: 'AK机器',
            description: '开局就全知识点 +35%，并且每 AC 一题再额外 +10% 全知识点（最多 5 次）。',
            color: '#00C853',
            prob: 0.0,
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    ...
                    student._talent_state = student._talent_state || {};
                    if (eventName === 'contest_start' || eventName === 'mock_start') {
                        if (student._talent_state['AK机器']) return null;
                        student._talent_state['AK机器'] = { acCount: 0 };
                        ...
                        return 'AK机器启动：全知识点 +35%（赛中临时）';
                    }
                    if (eventName === 'contest_solve_problem') {
                        var st = student._talent_state['AK机器'];
                        ...
                        return 'AK机器：又 AC 一题，全知识点再 +10%（第' + st.acCount + '次）';
                    }
                } catch (e) { console.error('AK机器 天赋错误', e); }
                return null;
            }
        });

        // 心如止水：比赛开始时压力 -15，且所有判定按心理素质获得最高 +60% 通过率
        this.registerTalent({
            name: '心如止水',
            description: '心静则明：赛前压力 -15，比赛所有判定按心理素质获得最多 +60% 通过率。',
            color: '#00B8D4',
            prob: 0.0,
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    ...
                    return { action: 'boost_ability', amount: amount, message: student.name + ' 心如止水：判定通过率 +' + Math.round(amount * 100) + '%' };
                } catch (e) { console.error('心如止水 天赋错误', e); }
                return null;
            }
        });

        // 时之沙漏：比赛中段之后，全能力临时 +60%（每场一次）
        this.registerTalent({
            name: '时之沙漏',
            description: '越到后面越猛：比赛过半后全能力临时 +60%。',
            color: '#AA00FF',
            prob: 0.0,
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    ...
                    student._talent_state = student._talent_state || {};
                    if (student._talent_state['时之沙漏']) return null;
                    student._talent_state['时之沙漏'] = true;
                    ...
                    return '时之沙漏：比赛过半，全能力 +60%（赛中临时）';
                } catch (e) { console.error('时之沙漏 天赋错误', e); }
                return null;
            }
        });

        // 天命之子：比赛开始时 30% 概率全能力 +80%，否则压力 -10
        this.registerTalent({
            name: '天命之子',
            description: '老天爷赏饭吃：赛前有 30% 概率全能力 +80%，否则压力 -10。',
            color: '#FFD600',
            prob: 0.0,
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    ...
                    if (Math.random() < 0.30) {
                        ...
                        return '天命之子：老天爷赏饭！全能力 +80%（赛中临时）';
                    }
                    student.pressure = Math.max(0, Number(student.pressure || 0) - 10);
                    return '天命之子：今天不是幸运日，压力 -10';
                } catch (e) { console.error('天命之子 天赋错误', e); }
                return null;
            }
        });
