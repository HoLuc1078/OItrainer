...
(function(global){
  const TalentManager = {
    ...
    getTalentInfo(name){
      ...
      return { name: t.name, description: t.description, color: t.color, gradient: t.gradient, textColor: t.textColor };
    },

    /* ================= 天赋标签配色（统一渐变） =================
     * 所有天赋标签都由这里生成样式，保证"尽可能使用渐变"且风格一致。
     * 天赋定义里可以额外给 gradient: ['#a', '#b', ...] 指定多色渐变，
     * 否则自动由主色推导出一个相邻色相的渐变色。
     */
    _hexToRgb(hex){
      var h = String(hex == null ? '' : hex).trim();
      if(h.charAt(0) === '#') h = h.slice(1);
      if(h.length === 3) h = h.charAt(0)+h.charAt(0)+h.charAt(1)+h.charAt(1)+h.charAt(2)+h.charAt(2);
      if(h.length !== 6) return [43, 108, 176];
      var n = parseInt(h, 16);
      if(!isFinite(n)) return [43, 108, 176];
    ...
    _hslToHex(h, s, l){
      ...
      return this._rgbToHex(hue2rgb(pp, q, h + 1 / 3) * 255, hue2rgb(pp, q, h) * 255, hue2rgb(pp, q, h - 1 / 3) * 255);
    },
    _shiftHue(hex, deg){
      var hsl = this._rgbToHsl(hex);
      return this._hslToHex(hsl[0] + Number(deg || 0) / 360, Math.max(hsl[1], 0.35), Math.max(0.28, Math.min(0.62, hsl[2])));
    },
    _withAlpha(hex, a){
      var rgb = this._hexToRgb(hex);
      return 'rgba(' + rgb[0] + ',' + rgb[1] + ',' + rgb[2] + ',' + a + ')';
    },
    _readableTextColor(hex, fallback){
      if(fallback) return fallback;
      var hsl = this._rgbToHsl(hex);
      // 文本统一压暗，保证浅色底上也看得清
      return this._hslToHex(hsl[0], Math.max(hsl[1], 0.45), Math.min(hsl[2], 0.40));
    },
    // 生成天赋色点（小圆点）的背景值，同样走渐变
    getTalentDotBackground(name){
      var t = this.getTalent(name) || {};
      var color = t.color || '#2b6cb0';
      var stops = (Array.isArray(t.gradient) && t.gradient.length >= 2) ? t.gradient : [color, this._shiftHue(color, 46)];
      return 'linear-gradient(120deg, ' + stops.join(', ') + ')';
    },
    // 生成天赋标签的内联样式（背景/文字/描边全部走渐变配色）
    getTalentTagStyle(name){
      var t = this.getTalent(name) || {};
      var color = t.color || '#2b6cb0';
      var stops = (Array.isArray(t.gradient) && t.gradient.length >= 2) ? t.gradient.slice() : [color, this._shiftHue(color, 46)];
      var self = this;
      var bgStops = stops.map(function(c){ return self._withAlpha(c, 0.22); }).join(', ');
      var textColor = this._readableTextColor(stops[0], t.textColor);
      var borderA = this._withAlpha(stops[0], 0.55);
      var borderB = this._withAlpha(stops[stops.length - 1], 0.55);
      return 'background: linear-gradient(120deg, ' + bgStops + ');' +
             ' color: ' + textColor + ';' +
             ' border-color: ' + borderA + ' ' + borderB + ' ' + borderB + ' ' + borderA + ';';
    },
    ...
    tryAcquireTalent(student, multiplier = 1.0) {
      ...
      try{
        ...
        // 难度影响天赋获取概率：简单档翻倍（上限 1.0），专家档按 DIFFICULTY_BALANCE 的 talent 系数打折
        try{
          if(typeof window !== 'undefined' && window.game){
            const __d = Number(window.game.difficulty);
            const __base = isFinite(globalGetTalentProb) ? globalGetTalentProb : 0.5;
            if(__d === 1){
              globalGetTalentProb = Math.min(1.0, __base * 2.0);
            } else if(typeof getDifficultyBalance === 'function'){
              const __t = Number(getDifficultyBalance(__d).talent);
              if(isFinite(__t) && __t !== 1) globalGetTalentProb = Math.max(0, Math.min(1.0, __base * __t));
            }
          }
        }catch(e){}
  ...
  checkAndHandleTalentLoss(student) {
    ...
    registerDefaultTalents(game, utils){
      ...
      // helper: 判断某个 ctx 是否属于「模拟赛 / 网赛」而非正式比赛。
      // 模拟赛的 contestName 是 "CF Div.2" / "Atcoder ABC" 之类，而正式比赛是
      // CSP-S1 / CSP-S2 / NOIP / 省选 / NOI / CTT / CTS / IOI。
      // 之前若干天赋直接写死 ctx.contestName !== '模拟赛'，导致它们永远不会触发。
      var OFFICIAL_CONTEST_RE = /^(CSP-S1|CSP-S2|NOIP|省选|NOI|CTT|CTS|IOI)/;
      function isMockContest(ctx){
        if(ctx && ctx.isMock) return true;
        var n = (ctx && ctx.contestName) ? String(ctx.contestName) : '';
        if(!n) return true;               // 拿不到比赛名时按练习赛处理（兼容旧调用）
        return !OFFICIAL_CONTEST_RE.test(n);
      }
      ...
      this.registerTalent({
        ...
        this.registerTalent({
          ...
          handler: function(student, eventName, ctx){
            try{
              if(eventName !== 'contest_select_problem') return null;
              // 仅在模拟赛生效
              if(!isMockContest(ctx)) return null;
        ...
        beneficial: false,
          handler: function(student, eventName, ctx){
            try{
              ...
              // 外出集训时的压力翻倍效果
              if(eventName !== 'pressure_change') return null;
              if(!ctx || (ctx.source !== 'outing' && ctx.source !== 'overseas')) return null;
        ...
        this.registerTalent({
          ...
          handler: function(student, eventName, ctx){
            try{
              ...
              if(!(src === 'training' || src === 'task_training' || (typeof src === 'string' && src.indexOf('train') === 0))) return null;
              // 避免重复触发
              if(ctx && ctx.preview) return null;   // 压力预览阶段不产生副作用
        ...
        this.registerTalent({
          ...
          handler: function(student, eventName, ctx){
            try{
              if(eventName !== 'pressure_change') return null;
              // 训练相关事件源：task_training / extra_task_training / training / extra_train
              const src = (ctx && ctx.source) ? String(ctx.source) : '';
              if(src.indexOf('train') < 0) return null;
              const intensity = Number(ctx.intensity) || 1;
              if(intensity >= 3){
                // 在原有压力变化基础上减少 30%
        ...
        // ==================== 魔改 · 隐藏天赋 ====================
        // 隐藏天赋的共同特点：出现概率极低（或为 0，只能通过特殊途径获得），
        // 效果强度明显高于常规天赋，并且大多绑定在比赛中临时生效。
        ...
        this.registerTalent({
            ...
            color: '#FFB6C1',
            gradient: ['#ff0000', '#ffa500', '#ffff00', '#00ff00', '#0000ff', '#4b0082', '#ee82ee'],
            textColor: '#a21caf',
            prob: 0.01,
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    ...
                    student._talent_state = student._talent_state || {};
                    // 同一场比赛只发动一次，避免多次叠加
                    if (student._talent_state['嬲选手']) return null;

                    const thinkingBonus = Number(student._base_thinking || 0) * 0.5;
                    const codingBonus   = Number(student._base_coding || 0) * 0.5;
                    const mentalBonus   = Number(student._base_mental || 0) * 0.5;
        ...
        this.registerTalent({
            ...
            handler: function (student, eventName, ctx) {
                try {
                    if (eventName !== 'contest_start' && eventName !== 'mock_start') return null;
                    student._talent_state = student._talent_state || {};
                    if (student._talent_state['珂朵莉']) return null;

                    const dsBonus = Number(student.knowledge_ds || 0);
                    student.applyTempModifier('knowledge_ds', student.getTempModifier('knowledge_ds') + dsBonus);

                    student._talent_state['珂朵莉'] = true;
                    return '珂朵莉发动：数据结构能力翻倍（赛中临时）';
                } catch (e) {
        ...
        // 世界上最幸福的女孩：每参加一场正式比赛，永久叠加全能力 +10%
        this.registerTalent({
            name: '世界上最幸福的女孩',
            description: '我永远喜欢珂朵莉。',
            color: '#ff0000',
            gradient: ['#ff0000', '#ff7ab6', '#0000ff'],
            textColor: '#c0392b',
            prob: 0.0,
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    ...
                    if (!student._permanent_buffs) student._permanent_buffs = {};
                    let buff = student._permanent_buffs['世界上最幸福的女孩'];
                    if (!buff) {
                        // 首次触发时快照原始数值，后续一律基于快照计算，避免重复叠乘
                        buff = student._permanent_buffs['世界上最幸福的女孩'] = {
                            stack: 0,
                            base: {
                                thinking: Number(student._base_thinking || 0),
                                coding: Number(student._base_coding || 0),
                                knowledge_ds: Number(student.knowledge_ds || 0),
                                knowledge_math: Number(student.knowledge_math || 0)
                            }
                        };
                    }
                    buff.stack += 1;
                    const mult = 1 + buff.stack * 0.1;

                    student._base_thinking = buff.base.thinking * mult;
                    student._base_coding   = buff.base.coding * mult;
                    student.knowledge_ds   = buff.base.knowledge_ds * mult;
                    student.knowledge_math = buff.base.knowledge_math * mult;
                    student.pressure = Math.max(0, Number(student.pressure || 0) - 5);

                    return '世界上最幸福的女孩发动（第' + buff.stack + '层）：永久全能力 +' + (buff.stack * 10) + '%';
                } catch (e) {
                    console.error('世界上最幸福的女孩 天赋错误', e);
                }
                return null;
            }
        });
        ...
        // ---------- 魔改 · 强力隐藏天赋 ----------
        // 这些天赋 prob 均为 0，不会在训练中随机出现，
        // 只能通过「出境集训 → 天赋激发」花钱博取，或在少数国家的集训彩蛋中觉醒。

        // 样例过了就是过了：思考阶段偶尔出现"这题我好像会"的错觉，然后真的过了
        // 愚人节彩蛋式天赋：描述一律点到为止，不写真实概率
        this.registerTalent({
            name: '样例过了就是过了',
            description: '样例过了就是过了。',
            color: '#7C4DFF',
            ...
            handler: function (student, eventName, ctx) {
                try {
                    if (eventName !== 'contest_thinking' && eventName !== 'mock_thinking') return null;
                    if (getRandom() < 0.02) {
                        return { action: 'auto_pass_problem', message: student.name + ' 样例过了就是过了：随手一交，居然真的过了！' };
                    }
                } catch (e) { console.error('样例过了就是过了 天赋错误', e); }
                return null;
            }
        });

        // 暴力出奇迹：每过一个子任务档位，本场思维临时上升（越暴力越有感觉）
        this.registerTalent({
            name: '暴力出奇迹',
            description: '正解想不出来不要紧，暴力自有暴力的活法。',
            color: '#FF6D00',
            ...
            handler: function (student, eventName, ctx) {
                try {
                    if (eventName !== 'contest_pass_subtask' && eventName !== 'mock_pass_subtask') return null;
                    student._talent_state = student._talent_state || {};
                    var stack = Number(student._talent_state['暴力出奇迹'] || 0);
                    if (stack >= 12) return null;
                    student._talent_state['暴力出奇迹'] = stack + 1;
                    var bonus = Number(student._base_thinking || 0) * 0.06;
                    student.applyTempModifier('thinking', student.getTempModifier('thinking') + bonus);
                    return '暴力出奇迹发动（第' + (stack + 1) + '层）：思维 +6%（赛中临时）';
                } catch (e) { console.error('暴力出奇迹 天赋错误', e); }
                return null;
            }
        });

        // 你怎么知道我 AK 了：开局知识点抬一截，之后每 AC 一题再抬一截
        this.registerTalent({
            name: '你怎么知道我 AK 了',
            description: '你怎么知道我 AK 了？',
            color: '#00C853',
            gradient: ['#00C853', '#00E5FF'],
            prob: 0.0,
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    var K = ['knowledge_ds', 'knowledge_graph', 'knowledge_string', 'knowledge_math', 'knowledge_dp'];
                    student._talent_state = student._talent_state || {};
                    if (eventName === 'contest_start' || eventName === 'mock_start') {
                        if (student._talent_state['你怎么知道我 AK 了']) return null;
                        student._talent_state['你怎么知道我 AK 了'] = { acCount: 0 };
                        for (var i = 0; i < K.length; i++) {
                            var key = K[i];
                            var cur = Number(student[key] || 0) + student.getTempModifier(key);
                            student.applyTempModifier(key, student.getTempModifier(key) + cur * 0.35);
                        }
                        return '你怎么知道我 AK 了：开局先把知识点抬起来（赛中临时）';
                    }
                    if (eventName === 'contest_solve_problem') {
                        var st = student._talent_state['你怎么知道我 AK 了'];
                        if (!st || st.acCount >= 5) return null;
                        st.acCount += 1;
                        for (var j = 0; j < K.length; j++) {
                            var k2 = K[j];
                            var base = Number(student[k2] || 0) + student.getTempModifier(k2);
                            student.applyTempModifier(k2, student.getTempModifier(k2) + base * 0.10);
                        }
                        return '你怎么知道我 AK 了：又 AC 一题，知识点继续抬（第' + st.acCount + '次）';
                    }
                } catch (e) { console.error('你怎么知道我 AK 了 天赋错误', e); }
                return null;
            }
        });

        // 稳如老狗：赛前先松一口气，之后所有判定都吃心理素质的加成
        this.registerTalent({
            name: '稳如老狗',
            description: '不管发生什么，手都不抖。',
            color: '#00B8D4',
            gradient: ['#00B8D4', '#00C853'],
            prob: 0.0,
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    if (eventName === 'contest_start' || eventName === 'mock_start') {
                        student.pressure = Math.max(0, Number(student.pressure || 0) - 15);
                        return null;
                    }
                    if (eventName !== 'contest_check_subtask') return null;
                    var mental = Number(student._base_mental || 0);
                    var amount = Math.max(0, Math.min(0.6, (mental / 100) * 0.6));
                    if (amount <= 0.01) return null;
                    return { action: 'boost_ability', amount: amount, message: student.name + ' 稳如老狗：判定通过率 +' + Math.round(amount * 100) + '%' };
                } catch (e) { console.error('稳如老狗 天赋错误', e); }
                return null;
            }
        });

        // Deadline 是第一生产力：比赛过半之后才真正进入状态（每场一次）
        this.registerTalent({
            name: 'Deadline 是第一生产力',
            description: '前半场在发呆，后半场才是本人。',
            color: '#AA00FF',
            gradient: ['#AA00FF', '#FF4081'],
            prob: 0.0,
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    ...
                    if (!state) return null;
                    var tick = Number(state.tick || 0);
                    var total = Number(state.totalTicks || 0);
                    if (!total || tick * 2 < total) return null;
                    student._talent_state = student._talent_state || {};
                    if (student._talent_state['Deadline 是第一生产力']) return null;
                    student._talent_state['Deadline 是第一生产力'] = true;
                    var bonusT = Number(student._base_thinking || 0) * 0.6;
                    var bonusC = Number(student._base_coding || 0) * 0.6;
                    var bonusM = Number(student._base_mental || 0) * 0.6;
                    student.applyTempModifier('thinking', student.getTempModifier('thinking') + bonusT);
                    student.applyTempModifier('coding', student.getTempModifier('coding') + bonusC);
                    student.applyTempModifier('mental', student.getTempModifier('mental') + bonusM);
                    student.applyTempModifier('constmental', student.getTempModifier('constmental') + bonusM);
                    return 'Deadline 是第一生产力：时间过半，人突然就醒了（赛中临时）';
                } catch (e) { console.error('Deadline 是第一生产力 天赋错误', e); }
                return null;
            }
        });

        // 欧皇：赛前抽一次卡，出货就起飞，不出货也不算太亏
        this.registerTalent({
            name: '欧皇',
            description: '有人靠实力，有人靠血统。',
            color: '#FFD600',
            gradient: ['#FFD600', '#FF9100'],
            prob: 0.0,
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    if (eventName !== 'contest_start' && eventName !== 'mock_start') return null;
                    if (getRandom() < 0.30) {
                        var bT = Number(student._base_thinking || 0) * 0.8;
                        var bC = Number(student._base_coding || 0) * 0.8;
                        var bM = Number(student._base_mental || 0) * 0.8;
                        student.applyTempModifier('thinking', student.getTempModifier('thinking') + bT);
                        student.applyTempModifier('coding', student.getTempModifier('coding') + bC);
                        student.applyTempModifier('mental', student.getTempModifier('mental') + bM);
                        student.applyTempModifier('constmental', student.getTempModifier('constmental') + bM);
                        return '欧皇：十连出货，全能力 +80%（赛中临时）';
                    }
                    student.pressure = Math.max(0, Number(student.pressure || 0) - 10);
                    return '欧皇：这次没出，但心态还在，压力 -10';
                } catch (e) { console.error('欧皇 天赋错误', e); }
                return null;
            }
        });
