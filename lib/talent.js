  ...
  checkAndHandleTalentLoss(student) {
    ...
    registerDefaultTalents(game, utils){
      ...
      this.registerTalent({
        ...
        // 世界上最幸福的女孩：每场比赛永久叠加全能力+10%、压力-10%（不被清理）
        this.registerTalent({
            name: '世界上最幸福的女孩',
            description: '我永远喜欢珂朵莉！',
            color: '#ff0000', // 主色（用于非渐变 fallback）
            prob: 0.01, // 天赋出现概率（5%，可调整）
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    // 仅在比赛开始时触发（contest_start）
                    if (eventName !== 'contest_start') return null;

                    // 初始化状态存储（不使用 _talent_backup，避免被 __talent_cleanup__ 清理）
                    if (!student._permanent_buffs) {
                        student._permanent_buffs = {
                            '世界上最幸福的女孩': { stack: 0 } // 记录叠加层数
                        };
                    }
                    if (!student._permanent_buffs['世界上最幸福的女孩']) {
                        student._permanent_buffs['世界上最幸福的女孩'] = { stack: 0 };
                    }

                    const buff = student._permanent_buffs['世界上最幸福的女孩'];
                    buff.stack += 1; // 每场比赛叠加1层

                    // 定义受影响的所有能力属性（根据实际代码中的学生属性补充）
                    const abilities = [
                        'thinking', 'coding',
                        'knowledge_ds', 'knowledge_math', 'knowledge_algorithm',
                        'speed', 'accuracy'
                    ];
                    const baseMultiplier = 0.1; // 每层10%

                    // 应用能力提升（基于当前层数计算总加成）
                    abilities.forEach(attr => {
                        // 原始值（未受该天赋影响的基础值）
                        const original = student[`_original_${attr}`] ?? student[attr];
                        // 首次叠加时备份原始值，后续基于原始值计算
                        if (buff.stack === 1) {
                            student[`_original_${attr}`] = original;
                        }
                        // 总加成：1 + 层数×10%（如2层为1.2倍）
                        student[attr] = original * (1 + buff.stack * baseMultiplier);
                    });

                    // 应用压力降低（基于原始压力计算）
                    const originalStress = student._original_stress ?? student.stress;
                    if (buff.stack === 1) {
                        student._original_stress = originalStress;
                    }
                    student.stress = Math.max(0, originalStress * (1 - buff.stack * baseMultiplier)); // 压力不低于0

                    return `世界上最幸福的女孩发动（第${buff.stack}层）：全能力+${baseMultiplier * 100}%，压力-${baseMultiplier * 100}%`;
                } catch (e) {
