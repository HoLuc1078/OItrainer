...
(function(global){
  ...
  function generateInternationalStudents(chineseStudents){
    ...
    if(chinaCount > 0){
      avgAbility = Math.min(Math.round(avgAbility / chinaCount),300);
        avgThinking = Math.min(Math.round(avgThinking / chinaCount),300);
        avgCoding = Math.min(Math.round(avgCoding / chinaCount),300);
        for (const k in knowledgeAvg) knowledgeAvg[k] = Math.min(Math.round(knowledgeAvg[k] / chinaCount),300);
    } else {
