...
(function(global){
  ...
  const halfIndex = (game.week > halfBoundary_local) ? 1 : 0;
    ...
    // ===== 统计埋点（隐藏成就用；纯计数，与随机流无关） =====
    try{
      if(typeof trackAction === 'function'){
        trackAction('contestsPlayed');
        const totalMax = (config && Array.isArray(config.problems))
          ? config.problems.reduce((sum, p) => sum + Number(p.maxScore || 0), 0) : 0;
        careerResults.forEach(function(r){
          const sc = Number(r.score || 0);
          if(totalMax > 0 && sc >= totalMax) trackAction('perfectScores');
          if(r.medal === 'gold') trackAction('goldMedals');
          else if(r.medal === 'silver') trackAction('silvers');
          else if(r.medal === 'bronze') trackAction('bronzes');
          if(originalComp.name === 'IOI' && sc >= 400) trackAction('ioiFullScores');
        });
        // 头名：本场最高分（且确实是跟别人比出来的，不是一个人自嗨）
        const allScores = results.map(function(r){ return Number(r.score || 0); }).filter(function(v){ return isFinite(v); });
        if(allScores.length > 1){
          const best = Math.max.apply(null, allScores);
          if(best > 0 && careerResults.some(function(r){ return Number(r.score || 0) === best; })) trackAction('firstPlaces');
        }
      }
    }catch(e){ /* 统计失败不影响比赛结算 */ }
    console.log('【DEBUG】 handleCompetitionResults pushed record, careerCompetitions:', game.careerCompetitions);

    // 隐藏成就：NOI 金牌 / AK IOI 之类要在比赛结算后判定
  ...
  function getCompetitionCutoff(compName, totalMax){
    ...
    if(compName !== 'NOI'){
      ...
      const randomFactor = 1.0 + (getRandom() * 2 - 1) * fluctuation; // 0.95 ~ 1.05
  ...
  function handleMockContestResults(studentStates, config, isPurchased, diffIdx){
    ...
    /* 网络设施：模拟赛收益加成 */
    const networkMultiplier = (game.facilities && typeof game.facilities.getNetworkMockMultiplier === 'function')
      ? game.facilities.getNetworkMockMultiplier() : 1.0;
    ...
    for(let state of studentStates){
      ...
      // 应用付费倍数 + 网络设施加成到总增幅上限
      const knowledgeCap = CONTEST_MAX_TOTAL_KNOWLEDGE_GAIN * gainRatio.knowledge * gainMultiplier * networkMultiplier;
      const thinkingCap = CONTEST_MAX_TOTAL_THINKING_GAIN * gainRatio.thinking * gainMultiplier * networkMultiplier;
      const codingCap = CONTEST_MAX_TOTAL_CODING_GAIN * gainRatio.coding * gainMultiplier * networkMultiplier;
