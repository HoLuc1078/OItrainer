...
// 在lib/utils.js中，所有函数的最上方新增查重函数
function isNameDuplicate(name) {
    // 检查全局game对象中的学生列表是否有重名
    if (typeof game === 'undefined' || !game || !game.students) return false;
    return game.students.some(student => student.name === name);
}

// 找到已存在的generateName函数，修改为带循环去重的版本
function generateName(opts = {}) {
    ...
    do {
        ...
        if (getRandom() < minorityProb) {
            name = generateMinorityName(region);
        }
        ...
        if (getRandom() > 0.4) n += namesPool[uniformInt(0, namesPool.length - 1)];
        name = s + n;
    } while (isNameDuplicate(name)); // 关键：重复则重新生成
