...
const TASK_POOL = [
    ...
    { name: '阮·梅', difficulty: -1, boosts: [{ type: '数学', amount: 100 }, { type: '图论', amount: 100 }, { type: '字符串', amount: 100 }, { type: '数据结构', amount: 100 }, { type: '动态规划', amount: 100 }] },
];
 ...
 */
/** 当前这一局的 token（见 game.js 的 initGame）：缓存按局隔离，保证同种子可复现 */
function __taskMemoryKey() {
  try {
    const g = (typeof window !== 'undefined' && window.game) ? window.game : ((typeof game !== 'undefined') ? game : null);
    if (g && g.gameToken !== undefined && g.gameToken !== null) return 'game:' + g.gameToken;
  } catch (e) { }
  return 'anon';
}
...
function selectRandomTasks(count = 7) {
  ...
  if (typeof window !== 'undefined') {
    // 新的一局 → 清空"最近推荐过"的记忆，否则同种子重开会拿到不同的题目
    const memKey = __taskMemoryKey();
    if (window.__oiTaskMemoryKey !== memKey) {
      window.__oiTaskMemoryKey = memKey;
  ...
  const candidatesForRecommended = tasksWithScore
    .filter(item => item.absorptionRate >= 0.7) // 吸收率>70%
      ...
      .filter(item => !recentRecommended.includes('阮·梅')) // 排除阮梅
