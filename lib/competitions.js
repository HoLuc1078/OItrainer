...
(function(global){
  ...
  function buildContestConfig(contestDef){
    ...
    for(let i = 0; i < contestDef.numProblems; i++){
      ...
      // 全局难度增幅（数值平衡总开关）：作用于题目难度及由其派生的思维/代码难度
      problemDifficulty = Math.max(1, Math.floor(problemDifficulty * (typeof DIFFICULTY_MULTIPLIER !== 'undefined' ? DIFFICULTY_MULTIPLIER : 1.0)));
