...
// ===== 状态快照与差异汇总工具 =====
// 训练类天赋对本次训练收益的结算钩子
// - 摸鱼大师：取消压力的同时按 reduceKnowledgeRatio 削减本次知识增益
// - 厌学：reject_training 时清空本次训练的全部收益
// 注：Student.addKnowledge 会把负数夹成 0，所以这里直接改底层属性。
const TRAINING_KNOWLEDGE_KEY_MAP = { '数据结构': 'knowledge_ds', '图论': 'knowledge_graph', '字符串': 'knowledge_string', '数学': 'knowledge_math', '动态规划': 'knowledge_dp', 'DP': 'knowledge_dp' };
function applyTrainingTalentOutcome(s, talentResults, appliedBoosts, abilityGain){
  try{
    for (const r of (talentResults || [])) {
      const out = r && r.result;
      if (!out || typeof out !== 'object') continue;
      if (out.action === 'moyu_cancel_pressure' && out.reduceKnowledgeRatio) {
        const ratio = Math.min(1, Math.max(0, Number(out.reduceKnowledgeRatio) || 0));
        for (const b of (appliedBoosts || [])) {
          const key = TRAINING_KNOWLEDGE_KEY_MAP[b.type] || b.type;
          if (typeof s[key] === 'undefined') continue;
          s[key] = Math.max(0, Number(s[key] || 0) - Number(b.amount || 0) * ratio);
        }
        log(`${s.name} 摸鱼大师：本次知识增益 -${Math.round(ratio * 100)}%`);
      } else if (out.action === 'reject_training') {
        for (const b of (appliedBoosts || [])) {
          const key = TRAINING_KNOWLEDGE_KEY_MAP[b.type] || b.type;
          if (typeof s[key] === 'undefined') continue;
          s[key] = Math.max(0, Number(s[key] || 0) - Number(b.amount || 0));
        }
        if (abilityGain) {
          s.thinking = Math.max(0, Number(s.thinking || 0) - Number(abilityGain.thinking || 0));
          s.coding = Math.max(0, Number(s.coding || 0) - Number(abilityGain.coding || 0));
        }
        log(`${s.name} 厌学：本次训练收益被清空，压力 +15`);
      }
    }
  }catch(e){ console.error('applyTrainingTalentOutcome error', e); }
}
...
function trainStudentsWithTask(task, intensity) {
  ...
  if(!s || s.active === false) continue;
    ...
    for(const boost of results.boosts) {
      ...
      __appliedBoosts.push({ type: boost.type, amount: totalBoost });
    }
    ...
    applyTrainingTalentOutcome(s, __talentResults, __appliedBoosts, { thinking: thinkingGain, coding: codingGain });
